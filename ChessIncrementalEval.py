...
class ChessEngine: 
    ...
    def __init__(self,custom_board):
        ...
        self.board_value = -9999  # Inicializar
        ...
        eval_pawns = sum([pawntable[i] for i in self.board.pieces(chess.PAWN, chess.WHITE)])
        eval_pawns = eval_pawns + sum(
            [-pawntable[chess.square_mirror(i)] for i in self.board.pieces(chess.PAWN, chess.BLACK)])

        eval_knights = sum([knightstable[i] for i in self.board.pieces(chess.KNIGHT, chess.WHITE)])
        eval_knights = eval_knights + sum(
            [-knightstable[chess.square_mirror(i)] for i in self.board.pieces(chess.KNIGHT, chess.BLACK)])

        eval_bishops = sum([bishopstable[i] for i in self.board.pieces(chess.BISHOP, chess.WHITE)])
        eval_bishops = eval_bishops + sum(
            [-bishopstable[chess.square_mirror(i)] for i in self.board.pieces(chess.BISHOP, chess.BLACK)])

        eval_rooks = sum([rookstable[i] for i in self.board.pieces(chess.ROOK, chess.WHITE)])
        eval_rooks = eval_rooks + sum(
            [-rookstable[chess.square_mirror(i)] for i in self.board.pieces(chess.ROOK, chess.BLACK)])

        eval_queen = sum([queenstable[i] for i in self.board.pieces(chess.QUEEN, chess.WHITE)])
        eval_queen = eval_queen + sum(
            [-queenstable[chess.square_mirror(i)] for i in self.board.pieces(chess.QUEEN, chess.BLACK)])

        eval_king = sum([kingstable[i] for i in self.board.pieces(chess.KING, chess.WHITE)])
        eval_king = eval_king + sum(
            [-kingstable[chess.square_mirror(i)] for i in self.board.pieces(chess.KING, chess.BLACK)])
    ...
    def update_evaluation(self,move, side):
        ...
        if move.promotion is not None:
            if side:
                self.board_value = self.board_value + values[move.promotion - 1] - values[moving_piece - 1]
                self.board_value = self.board_value - tables[moving_piece - 1][move.to_square] \
