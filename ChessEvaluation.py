...
def update_evaluation(move, side):
    global board_value
    global board
...
def make_move(move):
    update_evaluation(move, board.turn)
    board.push(move)
    return move

def unmake_move():
    movement = board.pop()
    update_evaluation(movement, not board.turn)
    return movement
...
board = chess.Board("r1bqkb1r/pppppppp/2n5/6N1/4n3/8/PPPP1PPP/RNBQKB1R w KQkq - 0 4")
print(board)
print(board.is_valid())
start_time = time.time()
print("Encontrando movimiento...")
move_found = selectmove(1)
